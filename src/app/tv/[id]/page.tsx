...
import { getTVDetails, getTVCredits, getSimilarTV, getTVVideos } from "@/lib/tmdb";
...
export default function TVDetailPage() {
  ...
  const [similarTV, setSimilarTV] = useState<any>(null);
  ...
  useEffect(() => {
    async function fetchTVData() {
      try {
        ...
        if (!tvData || !(tvData as TVDetails).id) {
          throw new Error("TV show not found");
        }

        setTVShow(tvData as TVDetails);
      ...
      <LazyMotion features={domAnimation}>
        ...
        >
          ...
          >
            ...
            >
              {error || "We couldn't find details for this TV show. Please try another one."}
            </m.p>
            <m.div 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
  ...
  // Get similar TV shows (up to 10 for carousel)
  const similar = (similarTV?.results?.slice(0, 10) || []) as SimilarTV[];
  ...
  const avgRuntime = tvShow.episode_run_time && tvShow.episode_run_time.length > 0
      ...
      >
        ...
        <div className="container mx-auto px-6 py-8 -mt-20 relative z-20">
          <div className="max-w-6xl mx-auto">
                    ...
                    >
                      <div className="flex items-center gap-2 mb-3">
                        <m.div 
                          ...
                          variants={staggerContainer}
                        >
                          {tvShow.genres.map((genre: Genre, index) => (
                            <m.span
                              key={genre.id}
                ...
                >
                  <div className="relative aspect-video bg-black rounded-xl overflow-hidden">
                    <iframe
                      ...
                      title={trailer.name || "TV Show Trailer"}
            ...
            {/* Similar TV Shows Section with Carousel */}
            <m.section
              initial={{ opacity: 0, y: 60 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              >
                ...
                <h2 className="text-2xl font-bold text-white">Similar TV Shows</h2>
              </m.div>
              ...
              {similar.length > 0 ? (
                ...
                >
                  <Carousel opts={{ align: "start", loop: true }} className="w-full">
                    <CarouselContent>
                      {similar.map((similarShow: SimilarTV, index: number) => (
                        <CarouselItem
                          key={similarShow.id}
                          className="basis-1/2 sm:basis-1/3 md:basis-1/4 lg:basis-1/5 xl:basis-1/6"
                        >
                            ...
                            >
                              <div className="aspect-[2/3] relative overflow-hidden">
                                <Image
                                  ...
                                  alt={similarShow.name || similarShow.title || "No title"}
                                  fill
                                  className="object-cover group-hover:scale-110 transition-transform duration-700"
                                />
                              ...
                              <div className="p-3">
                                <h3 className="font-semibold text-white text-sm line-clamp-2 group-hover:text-cyan-300 transition-colors duration-500">
                                  {similarShow.name || similarShow.title}
                                </h3>
                                <div className="flex items-center justify-between mt-2">
                                  ...
                                  <span className="text-gray-400 text-xs bg-gray-700 px-2 py-1 rounded-full">
                                    {similarShow.first_air_date?.substring(0, 4) || "TBA"}
                                  </span>
                                </div>
                              </div>
                            </Link>
                ...
                <m.div 
                  ...
                  variants={cardVariants}
                >
                  <p className="text-gray-500 text-lg">No similar TV shows found.</p>
                </m.div>
              )}
            </m.section>
          </div>
